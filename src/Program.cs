...
{
    var connectionString = "Server=localhost;Database=shortener_db;User=root;Password=Arya@2005;";
    return new MySqlConnection(connectionString);
});

builder.Services.AddSingleton<UserReferralTracker>();
...
app.Use(async (context, next) =>
{
    if (context.Request.Path == "/shorten" && context.Request.Method == "POST")
    {
        ...
        // Track the referrer
        var referrerCode = context.Request.Form["referrerCode"];
        var referrerEmail = context.Request.Form["referrerEmail"];
        using (var referralTracker = app.Services.GetRequiredService<UserReferralTracker>())
        {
            await referralTracker.TrackReferralAsync(referrerCode, referrerEmail);
        }
    }
...
using (var db = new MySqlConnection(connString))
{
    ...
    db.Execute("CREATE TABLE IF NOT EXISTS user_referrals (ReferrerCode VARCHAR(10) PRIMARY KEY, ReferrerEmail VARCHAR(255), Timestamp DATETIME);");
}
